function solve() {
  ...
  const bestRestaurantText = document.querySelector(
    "#outputs > #bestRestaurant > p"
  );
  const bestRestaurantEmployees = document.querySelector(
    "#outputs > #workers > p"
  );
  let bestRestaurants = {};
  ...
  function displayBestRestaurant(restaurantName) {
    const sortedWorkers = Object.values(
    ...
    bestRestaurantText.textContent = `Name: ${restaurantName} Average Salary: ${restaurant.avgSalary.toFixed(
      2
  ...
  function onClick() {
    ...
    for (const restaurant of restaurants) {
      ...
      employees.split(", ").forEach((employee) => {
        ...
        bestRestaurants[restaurantName].employees.push({
          name: name,
          salary: salary,
        });
      ...
      bestRestaurants[restaurantName].avgSalary =
        bestRestaurants[restaurantName].totalSalary /
        bestRestaurants[restaurantName].employees.length;
    }
